...
const PILLAR_COLORS = {
  Authority: "bg-[#ede0f8] text-[#40086d] border-[#dccaf4]",
...
export default function CalendarView({
  ...
  return (
    <div className="bg-white rounded shadow-sm border border-[rgba(30,30,30,0.1)]">
      {/* Top controls — sticky */}
      <div className="sticky top-0 z-10 bg-white flex items-center justify-between p-4 border-b border-[rgba(30,30,30,0.1)]">
        <div className="flex items-center gap-3">
          <button
            onClick={goToPrevMonth}
            className="px-4 py-2 rounded border border-[rgba(30,30,30,0.1)] text-gray-700 text-sm hover:bg-[#f6f6f6] transition"
          >
            Previous
          </button>
        ...
        <div>
          <select
            ...
            className="bg-white border border-[rgba(30,30,30,0.1)] rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#40086d] text-gray-700"
          >
      ...
      <div className="bg-white border-t border-gray-100">
        {/* Day headers — sticky */}
        <div className="sticky top-[69px] z-10 grid grid-cols-7 border-b border-[rgba(30,30,30,0.1)] bg-[#f6f6f6]">
          {DAYS.map((d) => (
            <div
              key={d}
        ...
        <div className="grid grid-cols-7">
          {cells.map((dateStr, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`min-h-[120px] border-r border-b border-[rgba(30,30,30,0.08)] last:border-r-0 relative ${
                  isToday ? "bg-[#ede0f8]/30" : ""
                }`}
              >
                <div className="p-2">
                  {dayNum && (
                    <div
                      className={`text-sm font-semibold mb-1 ${
                        isToday
                          ? "w-7 h-7 rounded-full bg-[#40086d] text-white flex items-center justify-center"
                          : "text-gray-700"
                      }`}
                    >
                      {dayNum}
